...
resource "terraform_data" "check_orgs" {
  ...
  lifecycle {
    precondition {
      ...
      error_message = "checks reference unknown orgs: ${join(", ", [for key, check in var.checks : "${key} -> ${check.org}" if !contains(var.org_keys, check.org)])}. Add the org (orgs.auto.tfvars plus its org_<key>.tf file) or fix the check."
    }
  }
}
